  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    + vec4(-1.5468478, -3.6171484, 0.24762098, 0.0);
     ...
     // Map to GlossIQ brand colors: Primary #23336E (deep blue) to Secondary #FEBD59 (golden)
     float primaryR = 0.137;  // #23336E
     float primaryG = 0.200;  
     float primaryB = 0.432;
     
     float secondaryR = 0.996; // #FEBD59
     float secondaryG = 0.741;
     float secondaryB = 0.349;
     
     vec3 finalColor = mix(
       vec3(primaryR, primaryG, primaryB),
       vec3(secondaryR, secondaryG, secondaryB),
       buf[0].x
     );
  ...
  return (
    <div ref={canvasRef} className="bg-black absolute inset-0 -z-10 w-full h-full" aria-hidden>
      ...
      >
       ...
       <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-glossiq-primary/20 via-transparent to-glossiq-secondary/10" />
    </div>
  );
}
...
export default function Hero({
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      {/* GlossIQ Logo Background */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="relative w-full h-full max-w-4xl max-h-4xl">
          <img 
            src="/GlossIQ_logo.png" 
            alt="GlossIQ Background" 
            className="absolute inset-0 w-full h-full object-contain opacity-8 scale-125"
            style={{
              filter: 'blur(0.5px)',
              mixBlendMode: 'screen',
          ...
          <img 
            src="/GlossIQ_logo.png" 
            alt="GlossIQ Background Layer" 
            className="absolute inset-0 w-full h-full object-contain opacity-4 scale-150"
            style={{
              filter: 'blur(2px)',
